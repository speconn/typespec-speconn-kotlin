...
function emitKotlin(program: Program, services: ServiceInfo[], outputDir: string): Promise<void[]> {
  ...
  for (const svc of services) {
    ...
    for (const rpc of svc.rpcs) {
      ...
      if (rpc.isStream) {
        client.push(`    suspend fun ${rpc.name}(request: ${reqName}): List<${resName}>`);
      } else {
    ...
    for (const rpc of svc.rpcs) {
      ...
      const resName = rpc.outputType?.name || "Unit";
      if (rpc.isStream) {
        client.push(`    override suspend fun ${rpc.name}(request: ${reqName}): List<${resName}> =`);
        client.push(`        unaryClient.stream<${reqName}, ${resName}>("${rpc.path}", request)`);
      } else {
        client.push(`    override suspend fun ${rpc.name}(request: ${reqName}): ${resName} =`);
        client.push(`        unaryClient.call<${reqName}, ${resName}>("${rpc.path}", request)`);
      }
    }
    client.push('}\n');
