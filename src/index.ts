...
function emitKotlin(program: Program, services: ServiceInfo[], outputDir: string): Promise<void[]> {
  ...
  for (const svc of services) {
    ...
    for (const m of svc.models) {
      ...
      types.push(`data class ${m.name}(${fieldLines.length > 0 ? "\n" + fieldLines.join(",\n") + "\n" : ""})\n`);
    }
    ...
    client.push("import speconn.StreamClient");
    client.push("import kotlinx.coroutines.flow.Flow");
    client.push("import okhttp3.OkHttpClient");
    client.push(`import ${pkg}.${typesBase}.*\n`);
    ...
    for (const rpc of svc.rpcs) {
      ...
      if (rpc.isStream) {
        client.push(`    fun ${rpc.name}(request: ${reqName}): Flow<${resName}>`);
      } else {
        client.push(`    suspend fun ${rpc.name}(request: ${reqName}): ${resName}`);
      }
    }
    client.push('}\n');
    client.push(`class ${svc.serviceName}ClientImpl(`);
    client.push(`    private val unaryClient: UnaryClient,`);
    client.push(`    private val streamClient: StreamClient,`);
    client.push(`) : ${svc.serviceName}Client {`);
    for (const rpc of svc.rpcs) {
      const reqName = rpc.inputType?.name || "Unit";
      const resName = rpc.outputType?.name || "Unit";
      if (rpc.isStream) {
        client.push(`    override fun ${rpc.name}(request: ${reqName}): Flow<${resName}> =`);
        client.push(`        streamClient.call("${rpc.path}", request)`);
      } else {
        client.push(`    override suspend fun ${rpc.name}(request: ${reqName}): ${resName} =`);
        client.push(`        unaryClient.call("${rpc.path}", request)`);
      }
    }
    client.push('}\n');
    client.push(`fun ${svc.serviceName}Client(httpClient: OkHttpClient, baseUrl: String): ${svc.serviceName}Client =`);
    client.push(`    ${svc.serviceName}ClientImpl(UnaryClient(httpClient, baseUrl), StreamClient(httpClient, baseUrl))\n`);
