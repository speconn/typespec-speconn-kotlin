...
function emitKotlin(program: Program, services: ServiceInfo[], outputDir: string): Promise<void[]> {
  ...
  for (const svc of services) {
    ...
    for (const rpc of svc.rpcs) {
      ...
      if (rpc.isStream) {
        client.push(`    suspend fun ${rpc.name}(request: ${reqName}, options: CallOptions = CallOptions()): List<${resName}>`);
      } else {
        client.push(`    suspend fun ${rpc.name}(request: ${reqName}, options: CallOptions = CallOptions()): ${resName}`);
      }
    }
    client.push('}\n');
    ...
    for (const rpc of svc.rpcs) {
      ...
      const resName = rpc.outputType?.name || "Unit";
      if (rpc.isStream) {
        client.push(`    override suspend fun ${rpc.name}(request: ${reqName}, options: CallOptions): List<${resName}> =`);
        client.push(`        unaryClient.stream<${reqName}, ${resName}>("${rpc.path}", request, options)`);
      } else {
        client.push(`    override suspend fun ${rpc.name}(request: ${reqName}, options: CallOptions): ${resName} =`);
        client.push(`        unaryClient.call<${reqName}, ${resName}>("${rpc.path}", request, options)`);
      }
    }
    client.push('}\n');
